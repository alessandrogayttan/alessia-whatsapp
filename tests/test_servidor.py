...
def test_webhook_envia_ack_antes_de_procesar(monkeypatch):
    ...
    monkeypatch.setattr(
        servidor, "marcar_leido_y_escribiendo", lambda mid: llamadas.append(("read", mid))
    )
    monkeypatch.setattr(
        servidor, "enviar_ack_inmediato", lambda tel: llamadas.append(("ack", tel))
    )
    monkeypatch.setattr(
        servidor,
...
def test_webhook_firma_invalida_rechazada(monkeypatch):
    monkeypatch.setattr(servidor, "verificar_firma_webhook", lambda body, firma: False)
    response = servidor.app.test_client().post(
        "/webhook",
        json={"entry": []},
